...
def apply_iddqd(engine, hero):
    engine.notify("IDDQD applied. Kill ’Em All!!!")
    engine.hero = Objects.IDDQD(hero)
...
def remove_effect(engine, hero):
    if "base" in dir(hero):
        ...
        else: engine.notify("Not Enough Gold")
    else: engine.notify("No effect to remove")
...
class MapFactory(yaml.YAMLObject):
    """используя абстрактную фабрику, раскидываем объекты по карте"""
    @classmethod
    def from_yaml(cls, loader, node):
        ...
        for obj_name in object_list_prob['enemies']:
            if not (obj_name in config): continue
            ...
            for i in range(config[obj_name]):
                ...
                while intersect:
                    intersect = False
                    if _map.Map[coord[1]][coord[0]] == wall:
                        intersect = True
                        coord = (random.randint(1, 39),
                                    random.randint(1, 39))
                        continue
                    for obj in _obj.objects:
            ...
            for i in range(41):
                for j in range(41):
                    if i == 0 or j == 0 or i == 40 or j == 40:
                        ...
                        self.Map[1][1] = self.Map[2][1] = self.Map[1][2] = self.Map[3][1] = self.Map[1][3] = floor1
